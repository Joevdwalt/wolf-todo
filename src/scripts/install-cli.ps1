<#+
.SYNOPSIS
Publishes Wolf Todo CLI and links it into a user executable directory.
#>
param(
    [string]$InstallDirectory = $env:WTODO_CLI_INSTALL_DIR,
...
if ([string]::IsNullOrWhiteSpace($InstallDirectory)) {
    if ($IsWindows) {
        $InstallDirectory = Join-Path $env:LOCALAPPDATA 'wtodo/cli'
    }
    elseif ($IsMacOS) {
        $InstallDirectory = Join-Path $HOME 'Library/Application Support/wtodo/cli'
    }
    else {
        ...
        $InstallDirectory = Join-Path $dataHome 'wtodo/cli'
    }
}
...
$executableName = if ($IsWindows) { 'wtodo.exe' } else { 'wtodo' }
...
if ($IsWindows) {
    $link = Join-Path $LinkDirectory 'wtodo.cmd'
    $marker = '@rem Wolf Todo CLI generated launcher'
    ...
    $link = Join-Path $LinkDirectory 'wtodo'
...
Write-Host "Installed Wolf Todo CLI to: $InstallDirectory"
